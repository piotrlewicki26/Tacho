...
// ═══════════════════════════════════════════════════════════════
function TachographPanel({tachoData,fileId}) {
    ...
    <div ref={rootRef} style={{fontFamily:"Inter",padding:0}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,flexWrap:"wrap",padding:"12px 16px",background:"#fff",borderBottom:"1px solid #E0E4E8"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          {extracted&&(
            <button onClick={()=>window.location.href='/reports/delegation?file_id='+fileId} style={{padding:"6px 14px",background:"linear-gradient(135deg,#1E88E5,#5C6BC0)",color:"#fff",border:"none",borderRadius:6,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"Inter",display:"flex",alignItems:"center",gap:6,boxShadow:"0 2px 8px rgba(30,136,229,0.3)"}}>
...
// ═══════════════════════════════════════════════════════════════
// APP — fetches DDD file from server, renders TachographPanel
// ═══════════════════════════════════════════════════════════════
function App() {
  const [tachoData, setTachoData] = useState(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (r) {
          setTachoData({ ...r, demo: false });
        } else {
          setErr('Nie wykryto danych aktywności w pliku. Format nieobsługiwany (.ddd, .v1b).');
        }
      } catch (ex) {
        setErr('Błąd wczytywania pliku: ' + ex.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div style={{ padding: '40px 20px', textAlign: 'center', fontFamily: 'Inter,sans-serif', color: '#5A6070' }}>
  ...
  return (
    <div style={{ background: 'transparent', fontFamily: 'Inter,sans-serif', width: '100%', overflow: 'hidden' }}>
      ...
      <TachographPanel tachoData={tachoData} fileId={TACHO_FILE_ID} />
    </div>
  );
}

const root = ReactDOM.createRoot(document.getElementById('tacho-analyzer-root'));
root.render(<App />);
</script>
